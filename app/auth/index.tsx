...
export default function HomeScreen() {
	const { tempSpot, clearTempSpot } = useTempSpotStore();
	...
	const [refetchSpots, setRefetchSpots] = useState<boolean>(false);
	const { user } = useAuthStore();

	useEffect(() => {
		if (tempSpot) {
			console.log('Setting form with tempSpot:', tempSpot);
			setModalVisible(true);
			setName(tempSpot.name || '');
			// Ensure CEP is converted to string regardless of its original type
			setCep(tempSpot.cep ? String(tempSpot.cep) : '');
			setAddress(tempSpot.address || '');
			setDistrict(tempSpot.district || '');
			setCity(tempSpot.city || '');
			setState(tempSpot.state || '');
		}
	}, [tempSpot]);
			...
			if (tempSpot) {
				...
				if (data) {
					Toast.show({
						type: "success",
						text1: "Atualização",
						text2: "Spot editado com sucesso",
					});
					setModalVisible(!modalVisible);
					setRefetchSpots(true);
				}
			} else {
				const { data, error } = await supabase
					.from("spots")
		...
		<SafeAreaView className="flex-1 bg-zinc-900 pt-10">
			<View className="w-full flex-1 bg-zinc-900 flex justify-start items-start p-6">
				...
				>
					...
					<ListSkateSpots refetchSpots={refetchSpots} />
									...
									>
										<View className="w-full flex justify-center items-center">
											<View className="w-full flex justify-center items-center bg-zinc-900 p-6">
												...
												>
													{!isSubmitting && (
														<Text className="text-white">
															{tempSpot ? "Editar SPOT" : "Cadastrar SPOT"}
														</Text>
													)}
													{isSubmitting && (
														<Text className="text-white">
												...
												<Pressable
													className="rounded-md w-full h-12 flex justify-center items-center mt-10"
													onPress={() => {
												...
												>
													<Text className="text-white">Cancelar Ação</Text>
												</Pressable>
											</View>
										</View>
									</ScrollView>
